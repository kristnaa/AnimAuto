...
## ManimConfig - Manim Community v0.20.1
...
ManimConfig’s attributes and properties.
...
main_utils.parse_args(), digest_parser(),
digest_file()

Notes

If args.config_file is a non-empty string, ManimConfig tries to digest the
contents of said file with digest_file() before
...
digest_parser() can process arbitrary parsers, built
...
digest_parser(), instead of calling this method
multiple times.
...
single file, whereas digest_file() can only process one
file at a time.
...
thus get_dir() is not appropriate; or when
...
ManimConfig. For example, the default value of video_dir
...
ManimConfig does store information about quality.
...
ManimConfig.get_dir() via kwargs.
...
property media_embed: bool | None¶
Whether to embed videos in Jupyter notebook.
...
property window_size: str | tuple[int, ...]¶
