...
_internal_value: a @property implemented on
ManimColor with the goal of keeping a consistent internal representation
...
_internal_space: a read-only @property implemented on
ManimColor with the goal of providing a useful representation which can be
...
_from_internal(): a @classmethod which converts an
...
Returns:
A list of interpolated ManimColor’s.
...
rgb (FloatRGB) – An RGB array of floats.

point (Point3D) – The location of the colored surface.

unit_normal_vect (Vector3D) – The direction in which the colored surface is facing.

light_source (Point3D) – The location of a light source which might illuminate the surface.
...
color1 (ManimColorT) – The first ManimColor.

color2 (ManimColorT) – The second ManimColor.
...
Parameters:
color (ManimColorT) – The ManimColor to invert.
...
Returns:
A random bright ManimColor.

Return type:
ManimColor

random_color()[source]¶
Return a random ManimColor.
...
Helper function for use in functional style programming. Refer to
ManimColor.from_rgb() and ManimColor.to_hex().
