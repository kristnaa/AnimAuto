# Graphing Number Line


---

## NumberLine - Manim Community v0.20.1
...
label_constructor (type[MathTex]) – Determines the mobject class that will be used to construct the labels of the number line.
...
Checks if the label is a VMobject, otherwise, creates a
label by passing label_tex to label_constructor.

Parameters:

label_tex (str | float | VMobject) – The label for which a mobject should be created. If the label already
...
dict_values (dict[float, str | float | VMobject]) – A dictionary consisting of the position along the number line and the mobject to be added:
...
direction (Point3DLike | None) – Determines the direction at which the label is positioned next to the line.
...
label_constructor (type[MathTex] | None) – The VMobject class that will be used to construct the label.
...
Adds DecimalNumber mobjects representing their position
...
Defaults to the output produced by get_tick_range()
...
Generates a positioned DecimalNumber mobject
...
direction (Vector3D | None) – Determines the direction at which the label is positioned next to the line.
...
## UnitInterval - Manim Community v0.20.1
...
## number_line - Manim Community v0.20.1
