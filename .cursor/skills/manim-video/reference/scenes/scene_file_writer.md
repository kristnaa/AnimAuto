...
## SceneFileWriter - Manim Community v0.20.1
...
For internal use only: takes a given frame in np.ndarray format and writes it to the stream
...
sound_file (StrPath) – The path to the sound file.
...
close_partial_movie_stream()[source]¶
Close the currently opened video container.
...
encode_and_write_frame(frame, num_frames)[source]¶
...
listen_and_write()[source]¶
For internal use only: blocks until new frame is available on the queue.

Return type:
None

next_section(name, type_, skip_animations)[source]¶
Create segmentation cut here.
...
open_partial_movie_stream(file_path=None)[source]¶
Open a container holding a video stream.
...
frame_or_renderer (PixelArray | OpenGLRenderer) – Pixel array of the frame.
...
## scene_file_writer - Manim Community v0.20.1
